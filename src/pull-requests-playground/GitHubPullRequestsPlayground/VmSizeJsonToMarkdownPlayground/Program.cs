...
// TODO: Handle conversion. keeping things simple for now.
namespace VmSizeJsonToMarkdownPlayground
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            string filePath = @"C:\dev\hackathons\6_9_22_vm_size_hack\vm_definitions-Dsv3.json";
            var res = VMSizeProcessor.ReadVMSizeJSON(filePath);
            res = VMSizeProcessor.ProcessVMSizeDataDTOs(res);

            Console.WriteLine("Generating markdown table from processed list");
            MdDocument doc = VMSizesJsonMarkdownWriter.GetNewIncludeDoc(res, new BasicVMSizesTableMarkdownSpec());
            Console.WriteLine(doc.ToString());
            doc.Save(filePath.Replace(".json", ".md"));
        }    
    }
